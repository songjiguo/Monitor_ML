...
void cos_init(void)
{
	...
	t1 = tsplit(cos_spd_id(), td_root, params2, strlen(params2), TOR_ALL, evt1, 0);
	printc("tid %d\n", t1);
	...
	t2 = tsplit(cos_spd_id(), t1, params1, strlen(params1), TOR_ALL, evt2, 0);
	if (t2 < 1) {
		printc("  split1 failed %d\n", t2); return;
	}

	t0 = tsplit(cos_spd_id(), t2, params0, strlen(params0), TOR_ALL, evt0, 0);
	if (t0 < 1) {
	...
	printc("2 split -> 2 read -> 2 release\n");

	/* t1 = tsplit(cos_spd_id(), td_root, params3, strlen(params3), TOR_ALL, evt1, 0); */
	/* ret1 = tread_pack(cos_spd_id(), t1, buffer, 1023); */
	/* if (ret1 > 0) buffer[ret1] = '\0'; */
	/* printv("read %d: %s\n", ret1, buffer); */
	/* assert(!strcmp(buffer, data2)); */
	/* assert(ret1 == strlen(data2)); */
	/* printc("  split with absolute addressing -> read\n"); */
	/* buffer[0] = '\0'; */
	...
	/* buffer[0] = '\0'; */
	/* printc("  writing to an existing file\n"); */
